		...
		<div class="container">
			<div class="row">
					...
					<div class="contact_form_container">
                        ...
                        <ul class="list-group col-lg-8" style="float: right;">
                            @if(Session::has('coupon'))
                                <li class="list-group-item">Sub Total: <span style="float:right;">Rs. {{ Session::get('coupon')['balance'] }}</span></li>
                            ...
                            @if(Session::has('coupon'))
                                @php 
                                    $s_total = Session::get('coupon')['balance'];
                                    $vat_amt = intval(($s_total*$vat)/100);
                                @endphp
                                <li class="list-group-item">Total Amount: <span style="float:right;">Rs. {{ Session::get('coupon')['balance'] + $charge + $vat_amt }}</span></li>
                            @else
                                @php 
                                    $s_total = Cart::Subtotal();
                                    $vat_amt = intval(($s_total*$vat)/100);
                                @endphp
                                <li class="list-group-item">Total Amount: <span style="float:right;">Rs. {{ Cart::Subtotal() + $charge + $vat_amt }}</span></li>
                            @endif
                        </ul>
					</div>
				</div>
                ...
                <div class="col-lg-5" style="border: 1px solid grey; padding: 20px; border-radius: 25px;">
					<div class="contact_form_container">
						<div class="contact_form_title text-center">Card Number</div>
                            ...
                            <form action="{{ route('stripe.charge') }}" method="post" id="payment-form">
                                ...
                                @if(Session::has('coupon'))
                                    @php 
                                        ...
                                        $vat_amt = intval(($s_total*$vat)/100);
                                    @endphp
                                    <input type="hidden" name="total" value="{{ Session::get('coupon')['balance'] + $charge + $vat_amt }} ">
                                @else
                                    @php 
                                        $s_total = Cart::Subtotal();
                                        $vat_amt = intval(($s_total*$vat)/100);
                                    @endphp
                                    <input type="hidden" name="total" value="{{ Cart::Subtotal() + $charge + $vat_amt }} ">
                                @endif
...
@section('js')
    <script type="text/javascript">
        ...
        var stripe = Stripe('pk_test_51IdYgwJ3UQJURVWz5ogSfF53rn53TJmnXQ0dRtK0tFGyXfqtdRfBxglpTYRegiQlow7r82RRy0XKv2cYvM1hJDVA0091Q5e97E');
