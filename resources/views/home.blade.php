                            ...
                            @foreach($order as $row)
                                <tr>
                                    ...
                                    <td scope="col">
                                        ...
                                        @elseif($row->status == 1)
                                            @if($row->payment_type == 'oncash')
                                                <span class="badge badge-info">Order Accepted</span>
                                            @else
                                    ...
                                    <td scope="col">
                                        @if($row->payment_type == 'oncash' && $row->status == 0)
                                            <a href="{{ URL::to('user/order/update/status/'.$row->id) }}" class="btn btn-sm btn-danger" id="cancel_order" title="Cancel Order"><i class="far fa-window-close"></i></a>
                                        @endif
                                        <a href="{{ URL::to('user/view/order/'.$row->id) }}" class="btn btn-sm btn-info" title="View Details"><i class="fas fa-eye"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>    
